...
class GetRecommendationsAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        prodt_id = df2[(df2['category'] == category) & (df2['assesment_type'] == ass_type) & (
                    df2['assesment_name'] == ass_name)][['product_id', 'product_name', 'product_description']]

        product_dict = dict(zip(prodt_id.product_id, prodt_id.product_name))
        product_desc_dict = dict(zip(prodt_id.product_id, prodt_id.product_description))
        final_list = []
        lis = product_dict.keys()
        program_id = self.wheel(list(lis))
        for i in program_id:
            new = product_dict[i]
            desc = product_desc_dict[i]
            data = {"programId": i, "programName": new, "programDescription": desc}
